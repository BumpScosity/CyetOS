...
void handle_key_press(unsigned char key, CursorPosition *cursor, int color, int shift, char *ascii_map, int ascii_map_size) {
    switch(key) {
        case 0x0E:
        handle_backspace(cursor, color);
        break;
        default:
        handle_character(key, cursor, color, shift, ascii_map, ascii_map_size);
        break;
    }
}
...
void handle_keyboard(CursorPosition *cursor) {
    ...
    while (1) {
        ...
        if (key & 0x01) { // check bit 0 of the status byte to see if a key has been pressed
            ...
            handle_key_press(key, cursor, color, shift, ascii_map, ascii_map_size);
        }
    }
}